...
FRAME      = "#30363d"
TITLE_TEXT = "#7d8590"
INK        = "#c9d1d9"    # single ascii color
CURSOR     = "#c9d1d9"

# reveal timing (one-shot; a cursor rasters top -> bottom)
ROW_DUR = 0.11
STAGGER = 0.11            # == ROW_DUR -> single cursor sweeping down
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">shlok@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'shlok@github:~$ whoami <tspan fill="{INK}">DevShlok</tspan></text>')
parts.append(f'<rect x="{PAD+179}" y="{status_y-12:.1f}" width="8" height="14" fill="{INK}">'
